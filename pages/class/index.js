...
function initClass() {
    for (key in classes) {
        for (key2 in classes[key]) {
            container.innerHTML += `
                <div class="card flex-r t-flex-c g20" data-time="${classes[key][key2]['time']}">
...
function filter(mode) {
    ...
    if (mode == "day") {
        ...
        card.forEach((e) => {
            e.dataset.time.split(" ")[0] == "14/12" ? e.style.display = "flex" : e.style.display = "none"
        })
    } else if (mode == "week") {
